...
void puTreeMaker(){
  ...
  vector<float> mcPU;
  setMCPU(dataYear,mcPU);
  cout<<"Setting PU according to data of "<<dataYear<<endl;
  
  if(dataYear==2016){
    dataPUhistName = "pileup_2016.root";
    outTrFile = "PU_profileTree2016.root";
    outTreeName ="weights_2016";
  }
  else if(dataYear==2017){
    dataPUhistName = "pileup_2017_data.root";
    outTrFile = "PU_profileTree2017.root";
    outTreeName ="weights_2017_runBCDEF";   
  }
  else if(dataYear==2018){
    dataPUhistName = "pileup_2018_partial.root";
    outTrFile = "PU_profileTree2018.root";
    outTreeName ="weights_2018";
  }
  else cout<<"Cannot find which data is used\n";
  outTrFile = mcSampleType + "_" + outTrFile;

  TFile *fd=new TFile(dataPUhistName);
  TH1F *hd=(TH1F*)fd->Get("pileup");
  // cout<<"No. of bins in data PU hist:"<<hd->GetNbinsX()<<endl;
  // cout<<"No. of MC PU scenarios:"<<mcPU.size()<<endl;
  
  TFile *fm=TFile::Open(mcFile);
  TTree *trmc=(TTree*)fm->Get("tnpEleIDs/fitter_tree");
  Int_t truePU=0;
  Float_t weight=0.;
  trmc->SetBranchAddress("truePU",&truePU);
  trmc->SetBranchAddress("weight",&weight);

  TFile *fpu=new TFile(outTrFile,"recreate");
  TTree *trpu=new TTree(outTreeName,outTreeName);
  double PUweight=0.,totWeight=0.;
  TBranch *b_PUweight = trpu->Branch("PUweight",&PUweight,"PUweight/D");
  TBranch *b_totWeight = trpu->Branch("totWeight",&PUweight,"totWeight/D");
  trpu->SetBranchAddress("PUweight",&PUweight);
  trpu->SetBranchAddress("totWeight",&totWeight);

  Long64_t nentries = trmc->GetEntries();
  int f=1;
  for (Long64_t i=0;i<nentries;i++){
    //    if(i>1136700) break;
    if(1.0*i/nentries > (0.1*f)){ cout<<10*f<<" %\n"; f++;}
    trmc->GetEntry(i);
    if(truePU < 0 || truePU > 99 || mcPU[truePU] == 0.){
      weight = 0.;
      totWeight = 0.;
    }
    else{
      PUweight = (hd->GetBinContent(hd->GetXaxis()->FindBin(truePU+0.0001)))/(hd->Integral()*(mcPU[truePU]));
      if(weight > 0.0) totWeight = PUweight;
      else totWeight = -PUweight;
    }
    // cout<<truePU<<" "<<weight<<endl;
    // cout<<"Data:"<<hd->GetBinContent(hd->GetXaxis()->FindBin(truePU+0.0001))<<" mc:"<<mcPU[truePU]<<" PU wt:"<<PUweight<<" TotWt:"<<totWeight<<endl;
    trpu->Fill();
  }
  cout<<"Total entries processed: "<<trpu->GetEntries()<<endl;
  fpu->cd();
  trpu->Write();
}


void setMCPU(int yearData, vector<float> &wts){
  if(wts.size()!=0) wts.resize(0);

  if(yearData==2016){
    wts = {1.78653e-05 ,2.56602e-05 ,5.27857e-05 ,8.88954e-05 ,0.000109362 ,0.000140973 ,0.000240998 ,
  ...
  else if(yearData==2017){
    wts = {3.39597497605e-05,    6.63688402133e-06,     1.39533611284e-05,     3.64963078209e-05,     6.00872171664e-05,     9.33932578027e-05,    0.000120591524486,
  ...
  else if(yearData==2018){
    wts = {4.695341e-10, 1.206213e-06, 1.162593e-06, 6.118058e-06, 1.626767e-05,
  ...
  else { cout<<"Which year's data is it???"<<endl;}
